...
! Jan 2026.
! Stolen from FISOC (ice ocean coupling) for testing Stokes approximations in
! an idealised setup.
! Note that densities for flotation are hard coded here.
...
!-------------------------------------------------------------------------------
REAL(KIND=dp) FUNCTION Ex5_Thickness_w(Model, node, x_dist) RESULT(Thickness)
  USE DefUtils
  USE MOD_ElmerGeom
  IMPLICIT NONE
  TYPE(Model_t),INTENT(IN)  :: Model
  ...
  REAL(KIND=dp)  :: UpperSurface, LowerSurface
